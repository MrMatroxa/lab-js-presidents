...
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter((president) => {
    if (president.party === "Democratic") return president;
  });
}

// Iteration 3 | Count Years in Office - reduce()
function countYearsInOffice(presidentsArr) {
  const yearsInOffice = presidentsArr.reduce((acc, curr) => {
    if (curr.leftOffice === null) return acc;
...
function getPresidentsBornAfter(presidentsArr, year) {
  return presidentsArr.filter((president) => {
    if (president.birthYear > year) return president
  })
}
...
function countRepublicanPresidents(presidentsArr) {
  const republicanPresidentSum = presidentsArr.reduce((acc, president) => {
    if (president.party === 'Republican') return acc + 1;
    //console.log("what acc ::", acc)
    return acc;
  }, 0)
  //console.log('this is', typeof republicanPresidentSum)
  return republicanPresidentSum;
}
